...
def get_tourney_matches(puuid):
    # Increased to 50 to ensure we find enough shared games, still costs only 1 request
    url = f"{BASE_URL}/lol/match/v5/matches/by-puuid/{puuid}/ids?type=tourney&start=0&count=50"
...
def process_teams():
    ...
    for team in database:
        ...
        # 2. Setup existing games tracking
        ...
        for match_id, count in match_tally.items():
            # Keep game if 4+ players share it, and it's new
        ...
        # 4. Sort chronologically and STRICTLY keep only the last 10
        sorted_games = sorted(migrated_games, key=lambda x: x["id"], reverse=True)
        team["discovered_games"] = sorted_games[:10]
        
        print(f"Added {new_games_count} new games. Database capped at 10 most recent for {team['team_name']}.")
